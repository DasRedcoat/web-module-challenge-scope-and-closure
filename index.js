...
function inningScoreCB(){
    return Math.floor(Math.random() * 3); // random float, times 3, rounded down
}
console.log(inningScoreCB)
...
function finalScore(inningScoreCB, Num){
  let homeTotal = 0;
  let awayTotal = 0;
  for (let i = 0, i < Num; i++) {
    homeTotal += inningScoreCB();
    awaytotal += inningScoreCB();
  }
  return {
    Away: awayTotal;
    Home: homeTotal;
